...
object FlywayPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val flywayUrl =
      settingKey[String]("The jdbc url to use to connect to the database.")
    val flywayUser =
      settingKey[String]("The user to use to connect to the database.")
    val flywayPassword =
      settingKey[String]("The password to use to connect to the database.")
    ...
    val flywayBaselineVersion = settingKey[String](
    ...
    val flywaySkipDefaultResolvers = settingKey[Boolean](
      "Whether default built-in resolvers should be skipped. (default: false)"
    )
    val flywayEncoding =
      settingKey[String]("The encoding of Sql migrations. (default: UTF-8)")
    val flywaySqlMigrationPrefix =
      settingKey[String]("The file name prefix for Sql migrations (default: V)")
    val flywayRepeatableSqlMigrationPrefix = settingKey[String](
      "The file name prefix for repeatable sql migrations (default: R)"
    )
    val flywaySqlMigrationSeparator = settingKey[String](
      "The file name separator for Sql migrations (default: __)"
    )
    val flywaySqlMigrationSuffixes = settingKey[Seq[String]](
    ...
    val flywayCleanDisabled = settingKey[Boolean](
      "Whether to disable clean. This is especially useful for production environments where running clean can be quite a career limiting move. (default: false)"
    )
    ...
    val flywaySkipDefaultCallbacks = settingKey[Boolean](
    ...
    //*********************
    val flywayIgnoreMissingMigrations = settingKey[Boolean](
      "Ignores missing migrations when reading the metadata table. (default: false)"
    )
    ...
    val flywayPlaceholderReplacement = settingKey[Boolean](
      "Whether placeholders should be replaced. (default: true)"
    )
    val flywayPlaceholders = settingKey[Map[String, String]](
      "A map of <placeholder, replacementValue> to apply to sql migration scripts."
    )
    val flywayPlaceholderPrefix =
      settingKey[String]("The prefix of every placeholder. (default: ${ )")
    val flywayPlaceholderSuffix =
      settingKey[String]("The suffix of every placeholder. (default: } )")
    ...
    val flywayValidateOnMigrate = settingKey[Boolean](
    ...
    // flyway tasks
    //*********************
    val flywayMigrate = taskKey[Unit](
      "Migrates of the configured database to the latest version."
    )
    ...
    val flywayBaseline = taskKey[Unit](
      "Baselines an existing database, excluding all migrations up to and including baselineVersion."
    )
    val flywayRepair = taskKey[Unit]("Repairs the metadata table.")
    val flywayDefaults = taskKey[FluentConfiguration](
      "Default configuration. This task is used to help resolve classpaths properly"
    )
  }
  ...
  private implicit class FluentConfigurationyOps(
    ...
    def configure(config: ConfigMigrate): FluentConfiguration = {
      val ignoreFutureMigrations =
        if (config.ignoreFailedMigrations) true
        else config.ignoreFutureMigrations

      var ignorePatterns = List[String]()

      if (config.ignoreMissingMigrations) {
        ignorePatterns = ignorePatterns :+ "Missing"
      }
      if (config.ignoreFailedMigrations) {
        ignorePatterns = ignorePatterns :+ "Failed"
      }
    ...
    def configureSysProps(config: ConfigDataSource): FluentConfiguration = {
      ...
      System.getProperties.asScala
        .filter(e => e._1.startsWith("flyway"))
        .foreach(e => props.put(e._1, e._2))
      config.asProps
        .filter(e => !sys.props.contains(e._1))
        .foreach(e => props.put(e._1, e._2))
  ...
  private object FlywaySbtLog extends Log {
    ...
    def notice(message: String): Unit = {
      streams foreach (_.log.debug(message))
    }
    ...
    def error(message: String): Unit = {
      streams foreach (_.log.error(message))
    }
